...
MODEL_PATH = "yolov5"
MIN_CONF = 0.4   #Zayıf algılamaları filtrelemek içi Güven değeri en az MIN_CONF olan nesneleri alacağız.
NMS_THRESH = 0.2 #NMS (Non Maximum Suppression)=
...
def detect_people(frame, net, ln, personIdx=0):
	...
	confidences = []
	class_ids=[]
	rows = layerOutputs[0].shape[1]
	image_height, image_width = frame.shape[:2]
	x_factor = image_width / 640 #640/640=1
	y_factor =  image_height / 640 #480/640=0.75
	toplam=0
	sayac=0
	...
	for r in range(rows):
		row=layerOutputs[0][0][r]
		confidence = row[4]

		
			
			# Nesne varsa ve Güven 0.5(MIN_CONF) ten büyük ise
		if confidence > NMS_THRESH:
                        classes_scores = row[5:]
                      # Maksimum sınıf puanı endeksini alın.
                        class_id = np.argmax(classes_scores)
                        if(classes_scores[class_id]>MIN_CONF):
                            ...
                            centroids.append((int(x),int(y),int(ort)))
...
LABELS = open(labelsPath).read().rstrip('\n').split('\n')
...
weightsPath = os.path.sep.join([MODEL_PATH, "yolov5s.onnx"]) 
#configPath = os.path.sep.join([MODEL_PATH, "coco128.yaml"])

# COCO veri setinde eğitilmiş YOLO nesne dedektörümüzü yüklüyoruz (COCO veri setinde 80 sınıf varken, biz sadece 1 sınıf kullanacağız.)
print("[BİLGİ] YOLO diskten yükleniyor...")
net = cv2.dnn.readNet("yolov5/yolov5s.onnx")
#net = cv2.dnn.readNetFromONNX(weightsPath)
...
#########################################################################################     
        ...
        if writer is not None:
                writer.write(frame)
print("[TOPLAM SÜRE] {:.4f} saniye".format(second))
